...
class UserController extends Controller
{
  ...
  {
    try {
      $enrollmentIds = Enrollment::where('user_id', Auth::id())->pluck('enrollment_id');
      return response()->json(['enrollments' => $enrollmentIds]);
    } catch (\Throwable $th) {
      Log::error("Erro ao buscar matrículas do usuário " . Auth::id() . ": " . $th->getMessage());

      return response()->json([
        'message' => 'Ocorreu um erro ao carregar suas matrículas. Tente novamente mais tarde.'
      ], 500);
    }
  }
}
